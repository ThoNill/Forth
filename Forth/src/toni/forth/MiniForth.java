...
public class MiniForth extends ForthContext {
	...
	private void doFetchMethod() throws IOException {
		...
		try {
			...
			throw new RuntimeException("Konnte die Methode " + name
					+ " in der Klasse " + cl.getName() + " nicht finden");
		} catch (SecurityException e) {
			throw new RuntimeException("Die Methode " + name
					+ " in der Klasse " + cl.getName() + " ist geschützt");
		}
	}
	...
	private void doCallConstructor() {
		...
		try {
			...
			throw new RuntimeException("Darf den Kontruktor in der Klasse " + m.getDeclaringClass().getName()
					+ "  nicht aufrufen");
		} catch (IllegalArgumentException e) {
			throw new RuntimeException("Falsche Argumente für den Kontruktor der Klasse "
					+ m.getDeclaringClass().getName() );

		} catch (InstantiationException e) {
			throw new RuntimeException("Der Kontruktor der Klasse "
					+ m.getDeclaringClass().getName() + " kann das Objekt nicht erzeugen");
		} catch (InvocationTargetException e) {
			throw new RuntimeException("Der Kontruktor der Klasse "
					+ m.getDeclaringClass().getName() + " passt nicht zur Klasse");			
		}
	}
	...
	private void doFetchConstructor() throws IOException {
		...
		try {
			...
			throw new RuntimeException("Konnte den Konstruktor in der Klasse " + cl.getName() + " nicht finden");
		} catch (SecurityException e) {
			throw new RuntimeException("Die Kontruktor in der Klasse " + cl.getName() + " ist geschützt");
		}
	...
	private void importClass() throws IOException {
		...
		try {
			...
			throw new RuntimeException("Die Klasse " + name
	...
	protected void createConstantWord(int position, Word ... zurLaufzeit) throws IOException {
		String name = input.nextToken();
		createConstantWord(name, position,zurLaufzeit);
	}
	
	protected void createConstantWord(String name,int position,Word ... zurLaufzeit) {
	...
	private void doIs() throws IOException {
		...
		if (w != null) {
			...
			throw new RuntimeException("Das Wort " + name
					+ " ist nicht vorhanden");
		}
	}
	...
	private void emit() {
		...
		try {
			...
			throw new RuntimeException("Das Zeichen mit dem Code "
					+ zeichenByte + " läßt sich nicht umwandeln");
		}
